...
const BUILTIN_C_PATH = path.join(process.cwd(), 'fastfetch', 'src', 'logo', 'builtin.c');
...
function generateLogos() {
  if (!fs.existsSync(LOGO_DIR)) {
    console.error('Fastfetch submodule not initialized or empty. Run git submodule update --init');
    process.exit(1);
  }
  ...
  for (const entry of logoEntries) {
    ...
    const names = [...namesRaw.matchAll(/"([^"]+)"/g)].map(m => m[1].toLowerCase());

    // Extract lines macro: FASTFETCH_DATATEXT_LOGO_MACOS
    const linesMatch = entry.match(/\.lines\s*=\s*(FASTFETCH_DATATEXT_LOGO_[A-Z0-9_]+)/);
    ...
    if (colorsMatch) {
      ...
      const colorTokens = [...colorsStr.matchAll(/(FF_COLOR_FG_[A-Z_]+)/g)].map(m => m[1]);
  ...
  for (const file of files) {
    ...
    let htmlLines = rawText.split('\n').map(line => {
      ...
      let out = '';
      let currentColorIndex = 0; // default 1 index, 0 in array
      ...
      for (let i = 0; i < parts.length; i++) {
        if (parts[i].match(/^\$[0-9]$/)) {
          ...
          if (i === 0 && config.colors[0]) out += `<span style="color: ${config.colors[0]}">`;
          out += parts[i];
          if (i === 0 && config.colors[0]) out += '</span>';
          else if (i > 0) out += '</span>';
        }
      }
      return out || `<span style="color: ${config.colors[0] || '#d4d4d4'}">${r}</span>`;
    });
...
`;
  ...
  console.log(`Successfully generated logos.ts with ${Object.keys(resultData).length} aliases from fastfetch submodule.`);
}
